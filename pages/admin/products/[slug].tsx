...
const ProductAdminPage: FC<Props> = ({ product }) => {
    ...
    const onFileSelected = async ({ target }: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            for (const file of target.files) {
                ...
                const { data } = await tesloApi.post<{ message: string }>('/admin/upload', formDataObj)
    ...
    const onSubmit = async (formData: FormData) => {
        if (formData.images.length < 2) return alert('Minimnos 2 imagenes')
        setIsSaving(true)
        try {
                    ...
                    <Grid item xs={12} sm={6}>
                        ...
                        <Box display='flex' flexDirection="column">
                            ...
                            <Input
                                // ref={fileInputRef}
                                // type='file'
                                // inputProps={{ multiple: true, accept: 'image/png, image/gif, image/jpeg' }}
                                // style={{ display: 'none' }}
